...
2. With the skill, reminder hooks are default, while blocking/global/network hooks require explicit confirmation.
3. With the skill, Codex and Claude templates are generated from separate references and not mixed.
4. With the skill, deterministic pre-commit/merge checks are routed to git hooks or CI instead of agent hooks.
5. Negative case: a React hook or product webhook request should not trigger agent-hook generation.
...
- `hook-template-builder/scripts/scan-project-hooks.sh /Users/flreey/Projects/myskills` - PASS; reports no common manifest, existing Claude settings, default recommendations, and existing-hook audit.
...
Deliverable for initial use. The skill enforces scan-first and confirmation-first behavior, defaults to reminder hooks, routes deterministic checks to CI/git hooks, and keeps Codex/Claude templates separate. Full A/B subagent validation is still recommended before publishing as a hardened public release.
