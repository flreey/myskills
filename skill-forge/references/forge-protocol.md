...
- **Artifact placement.** Decide from the interview's premises answer, and state the chosen path WITH the reason in the deliverable. Premises pin the skill to one codebase/product → that project's skills directory (`<project>/.claude/skills/<name>/`; Codex: `<project>/.codex/skills/` or `.agents/skills/`). Reusable across projects → the user's skills repo, symlinked into both global directories (`~/.claude/skills/<name>`, `~/.codex/skills/<name>`) so every project and both CLIs see it. A skill lives at exactly ONE layer — never copy or symlink the same name into both global and project scope (duplicate triggering, version drift). A project skill that later proves useful elsewhere gets promoted: move to the repo, strip project-specific premises, re-validate, symlink globally.
